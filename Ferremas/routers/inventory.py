...
@router.post("/", response_model=schemas.ProductResponse, tags=["Inventory"])
...
@router.put("/{product_id}", response_model=schemas.ProductResponse, tags=["Inventory"])
...
def read_inventory(skip: int = 0, limit: int = 10, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    if current_user.role not in ["bodega", "vendedor"]:
        raise HTTPException(status_code=403, detail="Operation not permitted")
